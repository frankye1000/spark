...
rdd = df.rdd
temp1 = rdd.map(lambda x: (x.person_id, x.politician_id))
...
df2.write.format("com.mongodb.spark.sql.DefaultSource").mode("append").option("database","fb_cleaned").option("collection", "pair").save()
